...
function preload() {
    ...
    worldMap = loadImage('5p/Blue_Marble_2002.png');
    testImage = loadImage('Berlin-Wall-Piece-NYC.jpg');
    font = loadFont('5p/lemon_milk/LEMONMILK-Medium.otf');
}

function setup() {
    //getting the canvase aligned to html file so that we can present both options
    // canvasWidth = myCanvas2D.width;
    // canvasHeight = myCanvas2D.height;
    let myCanvas2D = createCanvas(1920, 960); // Size of the world map image
    myCanvas2D.parent('myContainer2D');
    background(255);
    image(worldMap, 0, 0);
    ...
    for (let r = 0; r < table.getRowCount(); r++) {
        ...
        if (lat == 0 && long == 0) {
            ...
            // Save the points
            points.push({ x: x, y: y, r: 5, title: title, location: location, segment: segment }); // radius is 10 (half of 20) , image: images[imageName]
        } else {
